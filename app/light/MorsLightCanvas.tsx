...
export function MorsLightCanvas() {
  ...
  const scrollIllumRef = useRef(0);
  const applyLightRef = useRef<(() => void) | null>(null);
  const gyroEnableRef = useRef<(() => void) | null>(null);
  const [gyroPermissionNeeded, setGyroPermissionNeeded] = useState(() => {
    if (typeof window === "undefined") return false;
    const coarse = window.matchMedia?.("(pointer: coarse)").matches ?? false;
    const api = window.DeviceOrientationEvent as unknown as
      | { requestPermission?: () => Promise<string> }
      | undefined;
    return coarse && typeof api?.requestPermission === "function";
  });
  const [concept, setConcept] = useState<Concept>("品牌");
  ...
  useEffect(() => {
    ...
    void import("three-html-render/polyfill")
      ...
      .catch((polyfillError: unknown) => {
        ...
        if (active) {
          setError("当前浏览器不支持 HTML-in-Canvas，无法渲染灯光场景。");
        }
      });
  ...
  useEffect(() => {
    ...
    try {
      ...
      const errorTimer = window.setTimeout(() => {
        if (!disposed) {
          setError("这个页面需要 WebGL 才能渲染灯光场景。");
        }
      }, 0);
      return () => {
        disposed = true;
    ...
    // Scroll-driven illumination: 0 = the dark hanging-lamp scene, 1 = the room
    // fully flooded with light just before the page hands off to the content below.
    const sceneDark = new THREE.Color(0x010204);
    const sceneBright = new THREE.Color(0xe4e7ec);
    const pageDim = new THREE.Color(0xc5cad4);
    const pageLit = new THREE.Color(0xffffff);
    const backingDark = new THREE.Color(0x080a10);
    const backingLit = new THREE.Color(0xb9bdc7);
    const sceneColor = new THREE.Color();

    function applyLightState() {
      const currentLighting = lightingRef.current;
      const p = scrollIllumRef.current;
      const color = activeColorRef.current.set(currentLighting.color);
      const brightnessBoost = 1 + p * 4.6;
      const effectiveBrightness = currentLighting.enabled
        ? currentLighting.brightness * brightnessBoost
        : 0;

      spot.color.copy(color);
      spot.power = effectiveBrightness;
      spot.angle = THREE.MathUtils.degToRad(
        THREE.MathUtils.lerp(currentLighting.angle, 62, p),
      );
      spot.penumbra = THREE.MathUtils.lerp(0.88, 1, p);
      bulbLight.color.copy(color);
      bulbLight.power = currentLighting.enabled
        ? Math.max(18, currentLighting.brightness * 0.026) * (1 + p * 2)
        : 0;
      bulbMaterial.emissive.copy(color);
      bulbMaterial.emissiveIntensity = currentLighting.enabled
        ? 2.4 + currentLighting.brightness / 850 + p * 2.2
        : 0.04;
      glowMaterial.color.copy(color);
      glowMaterial.opacity = currentLighting.enabled
        ? Math.min(1, 0.52 + currentLighting.brightness / 4200 + p * 0.3)
        : 0;
      undersideMaterial.color.copy(color).multiplyScalar(0.18);
      undersideMaterial.emissive.copy(color);
      undersideMaterial.emissiveIntensity = currentLighting.enabled
        ? 0.22 + currentLighting.brightness / 7250 + p * 0.5
        : 0.03;

      ambient.intensity = 0.58 + p * 2.5;
      fillLight.intensity = 0.24 + p * 1.3;
      renderer.toneMappingExposure = 1.08 + p * 0.3;
      sceneColor.copy(sceneDark).lerp(sceneBright, p);
      scene.background = sceneColor;
      renderer.setClearColor(sceneColor, 1);
      pageMaterial.color.copy(pageDim).lerp(pageLit, p);
      backingMaterial.color.copy(backingDark).lerp(backingLit, p);
    }
    applyLightRef.current = applyLightState;
    applyLightState();

    const heroZone = canvas.closest(".hero-zone") as HTMLElement | null;
    let scrollFrame = 0;
    let lastIllum = -1;
    let htmlBgLight = false;

    function updateScrollIllumination() {
      scrollFrame = 0;
      if (disposed || !heroZone) return;
      const rect = heroZone.getBoundingClientRect();
      const range = Math.max(1, rect.height - window.innerHeight);
      const p = THREE.MathUtils.clamp(-rect.top / range, 0, 1);
      // Below the hero p is pinned at 1 — skip re-applying state and, more
      // importantly, skip wake() so the offscreen scene can go back to sleep.
      if (p === lastIllum) return;
      lastIllum = p;
      scrollIllumRef.current = p;
      applyLightState();
      // Keep the zone behind the sticky shell in step with the scene color so
      // collapsing mobile toolbars never expose a mismatched strip.
      heroZone.style.backgroundColor = `#${sceneColor.getHexString()}`;
      shellRef.current?.style.setProperty("--scroll-progress", p.toFixed(4));
      if (floodRef.current) {
        floodRef.current.style.opacity = THREE.MathUtils
          .smoothstep(p, 0.72, 1)
          .toFixed(4);
      }
      wake();
    ...
    function onScroll() {
      // Rubber-band overscroll shows the html background: keep it dark near the
      // dark hero and light near the light footer.
      const nearBottom =
        window.scrollY >
        (document.documentElement.scrollHeight - window.innerHeight) * 0.5;
      if (nearBottom !== htmlBgLight) {
        htmlBgLight = nearBottom;
        document.documentElement.style.backgroundColor = nearBottom ? "#fafafa" : "#010204";
      }
      if (!scrollFrame) scrollFrame = requestAnimationFrame(updateScrollIllumination);
    }

    // Mobile: tilting the phone tilts the simulated gravity vector, so the
    // hanging lamp leans and swings with the device (gyroscope interaction).
    const gyroTilt = { x: 0, z: 0 };
    let gyroBase: { beta: number; gamma: number } | null = null;
    let gyroActive = false;

    function onDeviceOrientation(orientationEvent: DeviceOrientationEvent) {
      let beta = orientationEvent.beta;
      let gamma = orientationEvent.gamma;
      ...
      // Drift the neutral pose toward how the phone is being held, so any
      // comfortable holding angle slowly becomes the resting position.
      gyroBase.beta += (beta - gyroBase.beta) * 0.004;
      gyroBase.gamma += (gamma - gyroBase.gamma) * 0.004;
      const tiltX = THREE.MathUtils.clamp(gamma - gyroBase.gamma, -30, 30);
      const tiltZ = THREE.MathUtils.clamp(beta - gyroBase.beta, -30, 30);
      gyroTilt.x = THREE.MathUtils.degToRad(tiltX);
      gyroTilt.z = THREE.MathUtils.degToRad(tiltZ) * 0.7;
      gyroActive = true;
      wake();
    }
    ...
    const orientationApi = window.DeviceOrientationEvent as unknown as
      ...
      | undefined;
    if (coarsePointer && orientationApi) {
      if (typeof orientationApi.requestPermission === "function") {
        // iOS requires an explicit user gesture before orientation events flow.
        gyroEnableRef.current = () => {
          orientationApi
            .requestPermission?.()
            .then((state) => {
    ...
    function stepPhysics() {
      if (gyroActive) {
        const gravityX = Math.sin(gyroTilt.x) * 9.81;
        const gravityZ = Math.sin(gyroTilt.z) * 9.81;
        gravity.set(
          gravityX,
          -Math.sqrt(Math.max(1, 9.81 * 9.81 - gravityX * gravityX - gravityZ * gravityZ)),
          gravityZ,
        );
      }
    ...
    function onPointerUp(event: PointerEvent) {
      ...
      if (event.type === "pointercancel") {
        // The browser reclaimed the pointer (touch scroll took over) — release
        // the lamp gently instead of flinging it on every scroll gesture.
        pulling = false;
        pullPointerId = -1;
        pullStrength = 0;
        previous.copy(position);
    ...
    return () => {
      ...
      applyLightRef.current = null;
      gyroEnableRef.current = null;
      pageTexture.dispose();
  ...
  useEffect(() => {
    if (!error) return;
    const zone = shellRef.current?.closest(".hero-zone");
    zone?.classList.add("is-static");
    return () => zone?.classList.remove("is-static");
  }, [error]);
  ...
  return (
    ...
    >
      <canvas ref={canvasRef} className="webgl-canvas" aria-label="灯光交互首页">
        <MorsPageSurface
      ...
      <div ref={floodRef} className="flood-overlay" aria-hidden="true" />
      {error ? null : (
        <div className="scroll-hint" aria-hidden="true">
          <span>向下滚动 · 点亮整间屋子</span>
          <i />
        </div>
      )}
      {gyroPermissionNeeded && !error ? (
        <button
          type="button"
          className="gyro-button"
          onClick={() => gyroEnableRef.current?.()}
        >
          开启重力感应 · 转动手机摆灯
        </button>
      ) : null}
